...
async def send_file(unzip_bot, c_id, doc_f, query, full_path):
    try:
        ...
        if cum == "video":
            ...
            await unzip_bot.send_video(chat_id=c_id, video=doc_f, caption="**Extracted by @anujedits76**", duration=int(vid_duration) if vid_duration.isnumeric() else 0, thumb=str(thumb))
        else:
            await unzip_bot.send_document(chat_id=c_id, document=doc_f, caption="**Extracted by @anujedits76**")
        os.remove(doc_f)
